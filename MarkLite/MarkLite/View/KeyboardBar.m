...
- (instancetype)init
{
    CGFloat w = kWindowWidth / (kDevicePhone ? 8 : 16);

    self = [super initWithFrame:CGRectMake(0, 0, kWindowWidth, w)];
...
- (void)createItem
{
    ...
    CGFloat w = kWindowWidth / maxCount;
    ...
    tipsBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, kWindowWidth, w)];
...
- (void)upload:(NSData*)data
{
    ...
    if ([Configure sharedConfigure].useLocalImage) {
         NSString *text = [NSString stringWithFormat:@"![MarkLite](%@)",[self saveImage:data]];
        [_editView insertText:text];
        [_editView becomeFirstResponder];
        return;
    }
...
- (NSString*)saveImage:(NSData*)imageData{
    NSString *dir = [documentPath() stringByAppendingPathComponent:@"images"];
    if (createDirectory(dir)) {
        NSString *path = [dir stringByAppendingPathComponent:[NSString uniqueString]];
